...
function EmptyPanel({ children }: { children: React.ReactNode }) {
  return <div className="flex min-h-32 items-center justify-center rounded-md border border-dashed border-gray-03 px-4 text-center font-mont text-xs leading-5 text-gray-05">{children}</div>;
}
...
export function CatalogTab({ entity, currency }: { entity: string; currency?: string | null }) {
  ...
  return <>
    ...
    <section className="min-w-0 rounded-md bg-white">
      ...
      <div className="grid grid-cols-1 gap-2 border-b border-gray-03 p-3 sm:grid-cols-2">
...
function VendorPricing({ item, insights, currency, allowed, loading, error }: { item: CatalogItem; insights?: CatalogItemInsights; currency?: string | null; allowed: boolean; loading: boolean; error: boolean }) {
  ...
  return <div className="space-y-4"><div className="rounded-md border border-gray-03 p-4"><p className="font-mont text-xs font-semibold text-black-01">Master reference</p><dl className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2"><Field label="Preferred vendor" value={item.preferred_vendor_name || "Not configured"} /><Field label="Standard price" value={formatMoney(item.standard_unit_price, currency)} /></dl><p className="mt-3 font-mont text-xs leading-5 text-gray-05">This is an indicative master default, not a vendor quotation.</p></div>{toArray(insights?.vendor_pricing).length ? <div className="overflow-x-auto rounded-md border border-gray-03"><table className="w-full min-w-[520px]"><thead><tr>{["Vendor", "Orders", "Quantity", "Observed price", "Latest"].map((label) => <th key={label} className="bg-[#F1F1F1] px-3 py-2 text-left font-mont text-[11px] font-semibold text-gray-01">{label}</th>)}</tr></thead><tbody>{toArray(insights?.vendor_pricing).map((row) => <tr key={row.vendor_id}><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs text-black-01">{row.vendor_name}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs tabular-nums text-black-01">{row.order_count}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs tabular-nums text-black-01">{row.total_quantity}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs tabular-nums text-black-01">{row.minimum_unit_price === row.maximum_unit_price ? formatMoney(row.minimum_unit_price, currency) : `${formatMoney(row.minimum_unit_price, currency)} – ${formatMoney(row.maximum_unit_price, currency)}`}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs text-black-01">{row.latest_order_date}</td></tr>)}</tbody></table></div> : <EmptyPanel>No approved purchase-order pricing is linked to this catalog item yet.</EmptyPanel>}</div>;
}
...
function PurchaseHistory({ insights, currency, allowed, loading, error }: { insights?: CatalogItemInsights; currency?: string | null; allowed: boolean; loading: boolean; error: boolean }) {
  ...
  return <div className="space-y-4"><dl className="grid grid-cols-1 gap-4 rounded-md border border-gray-03 p-4 sm:grid-cols-2"><Field label="Requisition lines" value={insights?.usage.requisition_line_count ?? 0} /><Field label="Linked stock items" value={insights?.usage.stock_item_count ?? 0} /></dl>{rows.length ? <div className="overflow-x-auto rounded-md border border-gray-03"><table className="w-full min-w-[520px]"><thead><tr>{["PO", "Vendor", "Date", "Quantity", "Unit price"].map((label) => <th key={label} className="bg-[#F1F1F1] px-3 py-2 text-left font-mont text-[11px] font-semibold text-gray-01">{label}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={`${row.purchase_order_id}-${row.document_number}`}><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs font-semibold text-primary">{row.document_number}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs text-black-01">{row.vendor_name}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs text-black-01">{row.order_date}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs tabular-nums text-black-01">{row.quantity}</td><td className="border-t border-gray-03 px-3 py-2 font-mont text-xs tabular-nums text-black-01">{formatMoney(row.unit_price, currency)}</td></tr>)}</tbody></table></div> : <EmptyPanel>No approved purchase orders are linked through this item’s requisition lines yet.</EmptyPanel>}</div>;
}
...
function CatalogForm({ entity, currency, initial, onClose }: { entity: string; currency?: string | null; initial?: CatalogItem; onClose: () => void }) {
  ...
  const valid = !!code.trim() && code.trim().length <= 40 && !!name.trim() && name.trim().length <= 200 && description.trim().length <= 255 && !!unit.trim() && unit.trim().length <= 24 && leadValid && Number.isInteger(price) && price >= 0;
  ...
  const save = async () => {
    ...
    try {
      const result = initial
        ...
        : await create({ entity, code: code.trim(), name: name.trim(), description: description.trim(), unit_of_measure: unit.trim(), category: category || undefined, preferred_vendor: vendor || undefined, default_expense_account: expense || undefined, default_tax_code: tax || undefined, lead_time_days: lead, standard_unit_price: price, is_active: active }).unwrap();
