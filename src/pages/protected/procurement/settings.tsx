  ...
  return <>
    <SettingsPanel title="Procurement defaults" description="New vendors and contracts use the payment term. New purchase orders use the delivery address when the buyer leaves it blank.">
      <div className="grid grid-cols-1 gap-4 px-4 py-4 sm:px-5 lg:grid-cols-2">
        <label className="min-w-0 font-mont text-xs font-semibold text-gray-01">Default payment terms<select className="mt-2 h-10 w-full rounded-md border border-gray-03 bg-white px-3 font-mont text-sm disabled:bg-gray-02" value={terms} onChange={(event) => setTerms(event.target.value)} disabled={!canUpdate}>{PAYMENT_TERMS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><span className="mt-1 block font-normal leading-5 text-gray-05">Applied to new vendor records.</span><SettingsConsumer consumer={consumers.default_payment_terms} /></label>
