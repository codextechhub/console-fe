...
} from "@/components/finance-ui/settings-layout";
...
function GeneralForm({ entityCode, values, history, canUpdate }: { entityCode: string; values: ProcurementSettingsValues; history: FinanceAuditLog[]; canUpdate: boolean }) {
  ...
  return <>
    <SettingsPanel title="Procurement defaults" description="New vendors and contracts use the payment term. New purchase orders use the delivery address when the buyer leaves it blank.">
      <div className="grid grid-cols-1 gap-4 px-4 py-4 sm:px-5 lg:grid-cols-2">
        <label className="min-w-0 font-mont text-xs font-semibold text-gray-01">Default payment terms<select className="mt-2 h-10 w-full rounded-md border border-gray-03 bg-white px-3 font-mont text-sm disabled:bg-gray-02" value={terms} onChange={(event) => setTerms(event.target.value)} disabled={!canUpdate}>{PAYMENT_TERMS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><span className="mt-1 block font-normal leading-5 text-gray-05">Applied to new vendor records.</span></label>
        <label className="min-w-0 font-mont text-xs font-semibold text-gray-01">Default delivery address<Textarea className="mt-2 min-h-24 bg-white font-mont text-sm" value={address} onChange={(event) => setAddress(event.target.value)} disabled={!canUpdate} placeholder="Enter the standard receiving location" maxLength={2000} /><span className="mt-1 block font-normal leading-5 text-gray-05">Buyers can still override this on a purchase order.</span></label>
      </div>
...
function MatchingForm({ entityCode, values, history, canUpdate }: { entityCode: string; values: ProcurementSettingsValues; history: FinanceAuditLog[]; canUpdate: boolean }) {
  ...
  return <>
    <SettingsPanel title="Three-way match policy" description="Tolerances apply to cumulative invoicing against ordered and received evidence. Values are percentages from 0 to 100.">
      <div className="grid grid-cols-1 gap-4 px-4 py-4 sm:grid-cols-2 sm:px-5">
        <label className="font-mont text-xs font-semibold text-gray-01">Quantity tolerance (%)<Input type="number" min="0" max="100" step="0.01" className="mt-2 bg-white" value={quantity} onChange={(event) => setQuantity(event.target.value)} disabled={!canUpdate} /><span className="mt-1 block font-normal leading-5 text-gray-05">Allowed overage against ordered and received quantity.</span></label>
        <label className="font-mont text-xs font-semibold text-gray-01">Price tolerance (%)<Input type="number" min="0" max="100" step="0.01" className="mt-2 bg-white" value={price} onChange={(event) => setPrice(event.target.value)} disabled={!canUpdate} /><span className="mt-1 block font-normal leading-5 text-gray-05">Allowed absolute unit-price difference from the PO.</span></label>
      </div>
      <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5"><div><p className="font-mont text-sm font-medium text-gray-01">Allow non-PO invoices</p><p className="mt-0.5 font-mont text-xs leading-5 text-gray-05">When off, invoices without purchase-order evidence receive a blocking match status.</p></div><Switch checked={allowNonPo} onCheckedChange={setAllowNonPo} disabled={!canUpdate} aria-label="Allow non-PO invoices" /></div>
