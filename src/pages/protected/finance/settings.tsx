            ...
            return (
              <div key={mapping.key} className="grid grid-cols-1 gap-3 px-4 py-4 sm:px-5 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.9fr)] lg:items-center">
                ...
                <div className="flex min-w-0 flex-col gap-2 sm:flex-row">
                  <select aria-label={`${mapping.label} account`} className="h-10 min-w-0 flex-1 rounded-md border border-gray-03 bg-white px-3 font-mont text-sm disabled:bg-gray-02" value={selected} disabled={!canUpdate} onChange={(event) => setChanges((current) => ({ ...current, [mapping.key]: event.target.value || null }))}>
...
function FinanceDocumentForm({ entityCode, values, history, canUpdate }: { entityCode: string; values: FinanceDocumentSettingsValues; history: FinanceAuditLog[]; canUpdate: boolean }) {
  ...
  return <>
    <SettingsPanel title="Billing defaults" description="These defaults are applied by the backend when a user leaves the corresponding field blank.">
      <div className="grid grid-cols-1 gap-4 px-4 py-4 sm:px-5 lg:grid-cols-2">
        <label className="font-mont text-xs font-semibold text-gray-01">Default invoice due days<Input className="mt-2 bg-white" type="number" min="0" max="365" step="1" value={dueDays} onChange={(event) => setDueDays(event.target.value)} disabled={!canUpdate} /><span className="mt-1 block font-normal leading-5 text-gray-05">Used for manual and fee-generated invoices without a due date.</span></label>
        <label className="font-mont text-xs font-semibold text-gray-01">Primary collection account<select className="mt-2 h-10 w-full rounded-md border border-gray-03 bg-white px-3 font-mont text-sm disabled:bg-gray-02" value={bankAccount} onChange={(event) => setBankAccount(event.target.value)} disabled={!canUpdate}><option value="">Automatic fallback</option>{values.bank_account_options.map((bank) => <option key={bank.id} value={bank.id}>{bank.name}{bank.bank_name ? ` · ${bank.bank_name}` : ""} · {bank.currency}</option>)}</select><span className="mt-1 block font-normal leading-5 text-gray-05">Printed as the payment destination on invoices and receipts.</span></label>
