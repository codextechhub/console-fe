// Record receipt (standalone, from the Receipts & Allocation screen) — pick the
// customer, the amount received and the bank/cash account it landed in. Posts the
// receipt (Dr bank, Cr AR); auto-allocate oldest-first is on by default, or leave
// it off to allocate manually in the receipt drawer. Naira in → integer kobo.
import { useState } from "react";
import { toast } from "sonner";
import { FormModal, FormField, CustomerPicker, AccountPicker } from "@/components/finance-ui";
import { Input } from "@/components/ui/input";
import { useRecordCustomerReceiptMutation } from "@/redux/services/finance/ar-api";

const selectCls = "h-9 w-full rounded-md border border-gray-03 bg-white px-2 font-mont text-sm focus:border-primary focus:outline-none";
const todayISO = new Date().toISOString().slice(0, 10);
const METHODS = ["BANK_TRANSFER", "CASH", "CARD", "CHEQUE", "ONLINE", "OTHER"] as const;
const methodLabel = (m: string) => m.replace("_", " ").toLowerCase().replace(/^\w/, (c) => c.toUpperCase());
const toKobo = (naira: string) => Math.round((parseFloat(naira) || 0) * 100);

export function RecordReceiptModal({ open, onOpenChange, entity }: {
  open: boolean; onOpenChange: (o: boolean) => void; entity: string;
}) {
  const [customer, setCustomer] = useState("");
  const [amount, setAmount] = useState("");
  const [date, setDate] = useState(todayISO);
  const [method, setMethod] = useState<string>("BANK_TRANSFER");
  const [account, setAccount] = useState("");
  const [reference, setReference] = useState("");
  const [autoAllocate, setAutoAllocate] = useState(true);
  const [record, { isLoading }] = useRecordCustomerReceiptMutation();

  const kobo = toKobo(amount);
  const canSubmit = !!customer && kobo > 0 && !!date && !!account;
  const reset = () => { setCustomer(""); setAmount(""); setDate(todayISO); setMethod("BANK_TRANSFER"); setAccount(""); setReference(""); setAutoAllocate(true); };
  const close = () => { reset(); onOpenChange(false); };

  const submit = async () => {
    try {
      const res = await record({
        entity, id: customer, amount: kobo, payment_date: date,
        method, deposit_account: account, reference: reference || undefined,
        auto_allocate: autoAllocate,
      }).unwrap();
      toast.success(res.message || "Receipt recorded.");
      close();
    } catch { /* central */ }
  };

  return (
    <FormModal
      open={open}
      onOpenChange={(o) => { if (!o) reset(); onOpenChange(o); }}
      title="Record receipt"
      description="Log money received from a customer and apply it to their open invoices."
      submitText="Record receipt"
      loading={isLoading}
      canSubmit={canSubmit}
      onSubmit={submit}
    >
      <FormField label="Customer" required>
        <CustomerPicker entity={entity} value={customer} onChange={setCustomer} placeholder="Type a customer name…" />
      </FormField>
      <div className="grid grid-cols-2 gap-3">
        <FormField label="Amount (₦)" required>
          <Input type="number" min="0" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="bg-white" />
        </FormField>
        <FormField label="Receipt date" required>
          <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="bg-white" />
        </FormField>
      </div>
      <FormField label="Method">
        <select value={method} onChange={(e) => setMethod(e.target.value)} className={selectCls} aria-label="Payment method">
          {METHODS.map((m) => <option key={m} value={m}>{methodLabel(m)}</option>)}
        </select>
      </FormField>
      <FormField label="Deposit account (bank / cash)" required>
        <AccountPicker entity={entity} value={account} onChange={setAccount} postableOnly accountType="ASSET" placeholder="Type a bank / cash account…" />
      </FormField>
      <FormField label="Reference">
        <Input value={reference} onChange={(e) => setReference(e.target.value)} placeholder="e.g. txn ref / cheque no" className="bg-white" />
      </FormField>
      <label className="flex items-center gap-2 font-mont text-sm text-gray-01">
        <input type="checkbox" checked={autoAllocate} onChange={(e) => setAutoAllocate(e.target.checked)} className="accent-primary" />
        Auto-allocate to open invoices (oldest first)
      </label>
    </FormModal>
  );
}
