...
export default function Support() {
  ...
  const cards = [
    { label: "Total tickets", value: d?.total, icon: TicketIcon },
    { label: "Open", value: d?.by_status.OPEN, icon: Inbox },
    { label: "In progress", value: d?.by_status.IN_PROGRESS, icon: Clock3 },
    { label: "Urgent", value: d?.by_priority.URGENT, icon: AlertCircle },
    { label: "Assigned to me", value: d?.assigned_to_me, icon: UserCheck },
    { label: "Resolved", value: d?.by_status.RESOLVED, icon: CheckCircle2 },
  ];
      ...
      <main className="px-4.5 py-6 lg:px-8">
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            {cards.map((c) => (
              <div className="rounded-xl border bg-white p-4 shadow-sm" key={c.label}>
                <c.icon className="size-5 text-primary" />
                <p className="mt-4 text-2xl font-semibold">{c.value ?? "—"}</p>
                <p className="text-xs text-gray-01">{c.label}</p>
              </div>
            ))}
          </div>
            ...
            {list.isLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {(list.data?.data ?? []).map((t) => (
                      <tr
                        key={t.id}
                        onClick={() => nav(routesPath.PROTECTED.SUPPORT.DETAIL(t.id))}
                      ...
                      >
                        ...
                        <td className="p-3">{t.requester.name}</td>
                        <td className="p-3 capitalize">{t.category.toLowerCase()}</td>
                        <td className="p-3">
                          <span
                            className={cn(
                              "font-medium",
                        ...
                        <td className="p-3">
                          <span className={cn("rounded-full px-2 py-1 text-xs", statusClass[t.status])}>
                            {t.status.replace("_", " ")}
                          </span>
                        </td>
                        <td className="p-3">{t.assignee?.name ?? "Unassigned"}</td>
                        <td className="p-3 text-xs text-gray-01">
                ...
                {!list.data?.data.length && (
                  <div className="py-16 text-center text-sm text-gray-01">No tickets match these filters.</div>
                )}
              </div>
            )}

            {(list.data?.pagination.totalPages ?? 0) > 1 && (
              <div className="flex justify-between border-t p-3 text-sm">
                <span>
                  Page {page} of {list.data?.pagination.totalPages}
                </span>
...
function CreateTicket({ close, done }: { close: () => void; done: (id: string) => void }) {
    ...
    <div className="fixed inset-0 z-50 grid place-content-center bg-black/30 p-4">
      <form onSubmit={submit} className="w-full max-w-xl rounded-xl bg-white p-6 shadow-xl">
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button disabled={isLoading} className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white">
