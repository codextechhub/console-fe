...
export function ConsoleSettingsLayout({
  ...
  return (
    <main className="min-w-0 space-y-5 px-4.5 pb-6 pt-14 text-black-01 sm:py-6">
      ...
      <div className="grid min-w-0 grid-cols-1 gap-5 md:grid-cols-[220px_minmax(0,1fr)]">
        <nav aria-label={`${title} sections`} className="min-w-0">
          <div className="flex max-w-full gap-2 overflow-x-auto pb-1 md:sticky md:top-4 md:block md:space-y-1 md:overflow-visible md:rounded-xl md:border md:border-gray-03 md:bg-white md:p-2">
            {sections.map((section) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "group flex min-w-max items-center gap-2.5 rounded-lg border border-gray-03 bg-white px-3 py-2.5 text-left transition-colors md:min-w-0 md:border-transparent",
                    active ? "border-primary/20 bg-primary/8 text-primary md:border-primary/20" : "text-gray-01 hover:bg-gray-02/50",
                  )}
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="mt-0.5 hidden truncate font-mont text-[10px] text-gray-05 md:block">{section.description}</span>
                  </span>
                </Link>
              );
            })}
...
export function SettingsRow({ label, description, value, badge, icon: Icon }: {
  ...
  return (
    <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:px-5">
      ...
      <div className="flex min-w-0 items-center gap-2 pl-11 sm:max-w-[48%] sm:justify-end sm:pl-0">
...
export function SettingsAuditHistory({ rows }: { rows: FinanceAuditLog[] }) {
  return (
