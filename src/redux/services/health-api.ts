...
export interface Overview { posture: { overall: string; label: string; critical: number; warning: number; active_incidents: number }; /** null until at least one real uptime rollup exists */ global_uptime: number | null; kpis: Record<"latency" | "traffic" | "errors" | "saturation", Signal>; services: Service[]; request_series: SeriesPoint[]; queues: Queue[]; active_incidents: Incident[] }
