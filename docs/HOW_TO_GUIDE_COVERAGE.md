...
| Getting started | Account activation and sign-in | `getting-started.activate-and-sign-in` | Medium | Published | Not required | Console product team | 2026-09-09 | Covers standard sign-in and random, revocable staff ID-card login. Invalid, replaced, locked, suspended, and missing card identifiers deliberately share one public message. A walkthrough would be unavailable before sign-in and could expose a personal activation or card link |
...
| Data imports and exports | Upload and resolve an import batch | `data.import-batch` | High | Published | Published | Console product team | 2026-09-09 | Covers active template use, tenant and dataset scope, upload metadata, structural and row validation, issue triage, explicit execution, monitoring, source proof, duplicate prevention, and leave warnings only while real wizard work exists. Walkthrough never uploads, validates, executes, deletes, or downloads data |
...
- The article covers choosing a role, module, and resource, searching field
  ...
  approval, the restricted update permission and its role change request, audit,
