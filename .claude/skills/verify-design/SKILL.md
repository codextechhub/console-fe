...
Playwright lives inside this skill's own folder (under gitignored `.claude/`),
so the project's `package.json` stays clean:
```bash
[ -d .claude/skills/verify-design/node_modules/playwright ] || \
  ( cd .claude/skills/verify-design && npm init -y >/dev/null 2>&1 && npm i playwright )
