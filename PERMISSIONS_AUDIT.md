...
#### Quick export from a list screen (`src/components/custom/quick-export-drawer.tsx`)

`QuickExportButton` / `QuickExportDrawer` are shared by every list screen with a
backend `ScreenBinding`. The button is gated on **both** keys the flow needs, via
`hasAllPermissions`:

| Element | Type | Permission Constant |
|---|---|---|
| Export button on a list screen | button | `P.VIEW_EXPORT_CATALOGUE` **and** `P.RUN_EXPORT` - hidden without both |
| Drawer · plan + estimate | drawer | `GET /exports/from-screen/` is gated `exports.catalogue.view`; a 403 renders `ForbiddenState` with the server's sentence |
| Drawer · Run export | button | `POST /exports/quick/` is gated `exports.run.create`, and re-checks `may_export_dataset` for the dataset behind the screen |

Both keys are required because holding only one produces a button that opens a
drawer it cannot submit - the same defect as a button with no handler. Sensitive
columns still need `P.EXPORT_SENSITIVE_FIELDS`; the quick export sends the
dataset's `default_columns` and the backend omits (and reports) anything the
caller may not export.

**Small exports run inline.** When the server's own estimate is at or under
`SYNC_EXPORT_MAX_BYTES` (4 MB) the run is produced during the request and the
drawer downloads it immediately - the button reads **Download export** rather
than Run export. The size check is server-side (`vs_exports.views.QuickExportView`);
the drawer's label is only a prediction, and a larger export silently falls back
to the queue. The download itself still goes through `GET /exports/files/<id>/download/`,
so `P.DOWNLOAD_EXPORT_FILE`, the downloader re-check and the download log all
apply exactly as they do on the Files screen - an inline run is not a way to take
a file without it being recorded.

**Columns are selectable.** The drawer offers every column the caller may export;
the field list on `from-screen` is filtered by the same sensitive-field gate the
catalogue uses, so a column needing `P.EXPORT_SENSITIVE_FIELDS` is never offered
to someone without it. Locked (identity) columns cannot be deselected.

Screens wired: `finance.invoices` · `finance.customers` · `finance.receipts` ·
`finance.gl_postings` · `payments.collections` · `payments.payouts` ·
`procurement.purchase_orders` · `procurement.vendors` ·
`procurement.vendor_invoices` · `procurement.requisitions` · `support.tickets` ·
`finance.expense_claims` · `admin.users` (Members **and** Invites - an invite is a
user row awaiting activation, so it needs no dataset of its own) ·
`admin.role_assignments` · `admin.sign_ins` · `workflow.instances` ·
`platform.schools`. Audit → Events keeps its own **Export filtered** flow
(`P.EXPORT_AUDIT`), which is a purpose-built compliance artifact, not a quick export.
Payments → Transactions Log exports whichever direction is selected
(`payments.collections` or `payments.payouts`); with neither chosen the button is
disabled with a reason, because one file cannot hold both shapes.

> **`platform.schools` is the one dataset whose base queryset is NOT the tenant
> boundary.** It serves `School.objects.all()`, mirroring `SchoolListView`, so the
> file matches the screen it is started from - the whole platform register. The
> boundary is the `platform.schools.view` key alone, which therefore must remain a
> platform-actor grant: giving it to a school-level role would let that role export
> every school's record. This was a reviewed, deliberate decision (2026-08-15); see
> the module docstring in `vs_schools/export_datasets.py`.
